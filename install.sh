...
do_export() {
    init_logger
    if [ "$EUID" -eq 0 ]; then
        error "export 需要以【普通用户】运行（要读取你的 ~/.config），不要用 sudo。"
        exit 1
    fi
    ...
    section "Export" "采集 niri 套件快照"
    info_kv "快照目录" "$BASE_DIR"

    # --- 3.1 包清单 ---
    log "扫描已安装的 niri 套件软件包..."
    ...
    info_kv "官方仓库包" "$(wc -l < "$SNAP_PKGLIST/official.txt") 个"
    info_kv "AUR 包" "$(wc -l < "$SNAP_PKGLIST/aur.txt") 个"
    if [ ${#missing[@]} -gt 0 ]; then
        warn "以下清单内软件包当前未安装，未写入快照: ${missing[*]}"
    fi
    ...
    info_kv "服务" "$(wc -l < "$SNAP_PKGLIST/services.txt") 项"
    ...
    if [ -f "$NIRI_KDL" ] && [ "$KEEP_TYPOS" -eq 0 ]; then
        ...
        if [ ${#fixed[@]} -gt 0 ]; then
            warn "已在快照副本中修正配置笔误（live 配置未改动；--keep-typos 可关闭此行为）:"
            for fx in "${fixed[@]}"; do echo -e "     ${H_YELLOW}· $fx${NC}"; done
            write_log "FIX" "${fixed[*]}"
        fi
    fi

    section "Export 完成" "快照已生成"
    info_kv "包清单" "$SNAP_PKGLIST/"
    info_kv "配置镜像" "$SNAP_CONFIG/"
...
check_root() {
    if [ "$EUID" -ne 0 ]; then
        error "restore 需要 root 权限，请使用: sudo ./replicate.sh restore"
        exit 1
    fi
}
...
detect_distro() {
    ...
    info_kv "发行版家族" "$DISTRO_FAMILY" "(ID=$id)"
}
...
detect_target_user() {
    ...
    if [ "$DRY_RUN" -eq 1 ]; then
        if [ ${#HUMAN_USERS[@]} -gt 0 ]; then
            ...
            log "[DRY-RUN] 自动选择目标用户: $TARGET_USER"
        else
            TARGET_USER="eilniri-dryrun"
            log "[DRY-RUN] 未检测到普通用户，使用占位用户名: $TARGET_USER"
        fi
    ...
    if [ ${#HUMAN_USERS[@]} -gt 0 ]; then
        ...
        echo -e "   ${H_YELLOW}>>> 检测到已有用户，选择配置部署目标:${NC}"
        local i
        ...
        echo -e "       [0] ${H_GREEN}创建新用户 ++${NC}"

        local idx
        while true; do
            echo -ne "   ${H_CYAN}输入序号 [0-${#HUMAN_USERS[@]}] (默认 ${default_user}, 30s 超时): ${NC}"
            if ! read -t 30 -r idx; then
                echo ""
                TARGET_USER="$default_user"
                log "超时，自动选择默认用户: $TARGET_USER"
                break
            fi
...
ensure_fzf() {
    ...
    pm_install fzf || { error "fzf 安装失败，无法继续。"; exit 1; }
    DRY_RUN="$_saved_dry"
}
...
stage_preflight() {
    ...
    if stage_done preflight; then
        log "Pre-flight 已完成，跳过（删除 .replicate_progress 可强制重跑）。"
        return
    fi
    if [ "$DISTRO_FAMILY" = arch ]; then
        exe pacman -Sy --noconfirm archlinux-keyring || warn "keyring 刷新失败，继续尝试。"
...
stage_apps_select() {
    section "应用选择" "TAB 切换 | 默认全选 | Ctrl-D 全不选 | Enter 确认"
    load_app_universe
    ...
    if [ "$DRY_RUN" -eq 1 ]; then
        ...
        log "[DRY-RUN] 自动全选所有应用: 仓库 ${#REPO_SEL[@]} 个, AUR ${#AUR_SEL[@]} 个"
        info_kv "已选" "仓库 ${#REPO_SEL[@]} 个" "AUR ${#AUR_SEL[@]} 个"
        return 0
    fi

    # 空清单守卫：fzf 空输入会返回非零且无提示，直接跳过避免误报"用户中止"
    if [ ${#lines[@]} -eq 0 ]; then
        warn "可用软件包列表为空，跳过应用安装阶段。"
        return 1
    fi
...
ensure_yay() {
    ...
    log "安装 AUR helper: yay ..."
    pm_install base-devel git || { error "base-devel/git 安装失败。"; return 1; }
    ...
    if [ "$DRY_RUN" -eq 0 ]; then
        ...
        log "[DRY-RUN] git clone yay-bin && makepkg -si"
        return 0
    fi
}
...
stage_services() {
    ...
    if [ ! -s "$svc_file" ]; then
        warn "未找到 services.txt，跳过服务启用。"
        return
    fi

    section "服务启用" "选择要启用的系统服务"
    local selected
    selected=$(awk '{print $1 "\t提供包: " $2}' "$svc_file" | \
        fzf_multi " 选择要启用的服务 ") || {
        warn "用户取消服务选择，跳过服务启用。"
        return
    }
    if [ -z "$selected" ]; then
        warn "未选择任何服务。"
    ...
    while IFS= read -r line; do
        ...
        if [ -n "$provider" ] && ! pkg_installed "$provider"; then
            log "补装服务提供包: $provider"
            erc=0
            pm_install "$provider" || erc=$?
            if [ "$erc" -ne 0 ] && [ "$erc" -ne "$DRY_RUN_RC" ]; then
                if [ "$provider" = dhcpcd ] && [ "$DISTRO_FAMILY" = rhel ]; then
                    MANUAL_ITEMS+=("dhcpcd —— RHEL 系建议使用 NetworkManager: systemctl enable --now NetworkManager")
                else
        ...
        if [ "$erc" -eq 0 ]; then
            ...
            if [ "$unit" = dhcpcd.service ] && [ "$DISTRO_FAMILY" = rhel ]; then
                MANUAL_ITEMS+=("dhcpcd.service —— 不可用，建议: systemctl enable --now NetworkManager")
            else
...
stage_dm() {
    ...
    if [ -n "$found" ]; then
        info_kv "DM 冲突" "$found" "已存在，跳过 ly"
    elif [ "$DRY_RUN" -eq 1 ]; then
        # dry-run：跳过交互默认 N；告知用户若想装可去掉 --dry-run
        log "[DRY-RUN] 跳过 ly 安装（默认 N）。若需实际安装，请去掉 --dry-run 重跑。"
        DRY_SVCS+=("ly@tty1")
    elif confirm "安装并启用 ly 显示管理器？[y/N] (默认 N, 20s):" "N" 20; then
        ...
        log "跳过 ly。无显示管理器时，tty 登录后执行 niri-session 即可进桌面。"
    fi
    stage_mark dm
}
...
stage_backup() {
    ...
    if [ "$DRY_RUN" -eq 1 ]; then
        log "[DRY-RUN] 跳过配置备份阶段。"
        stage_mark backup
        return
    fi

    section "配置快照" "部署前创建回滚点"
    local snap_cfg="$BASE_DIR/config"
    if [ ! -d "$snap_cfg/.config" ]; then
        warn "未找到 config/ 配置镜像，跳过备份。"
        stage_mark backup
        return
    fi
    ...
    if [ ${#targets[@]} -eq 0 ]; then
        log "没有已有配置需要备份（目标为空），跳过。"
    elif confirm "备份当前已有配置到快照以备回滚？[Y/n] (默认 Y, 15s):" "Y" 15; then
        if exe tar czf "$tgz" -C / "${targets[@]#/}" 2>/dev/null; then
            success "快照已保存: $tgz"
            info_kv "备份项数" "${#targets[@]} 个"
        else
            warn "快照创建失败，将继续部署（无回滚点）。"
        fi
    else
        log "跳过备份，继续部署。"
    fi
...
stage_configs() {
    ...
    if [ ! -d "$snap" ]; then
        warn "未找到 config/ 配置镜像，跳过部署。"
        return
    fi

    section "配置部署" "目标: $HOME_DIR"
    local ts
    ts=$(date +%Y%m%d-%H%M%S)
...
stage_verify() {
    ...
    [ "$cfg_errors" -eq 0 ] && success "配置审计通过。" || warn "$cfg_errors 个关键配置目录缺失。"
    stage_mark verify
}
...
print_summary() {
    section "汇总报告" "eilNiri restore"
    info_kv "已安装包" "${#INSTALLED_PKGS[@]} 个"
    info_kv "已跳过" "${#SKIPPED_PKGS[@]} 个"
    info_kv "已启用服务" "${#ENABLED_SVCS[@]} 项"
    if [ ${#ENABLED_SVCS[@]} -gt 0 ]; then
        printf "     ${DIM}%s${NC}\n" "${ENABLED_SVCS[@]}"
    fi
    if [ "$DRY_RUN" -eq 1 ]; then
        info_kv "[DRY-RUN] 将装包" "${#DRY_PKGS[@]} 个"
        if [ ${#DRY_PKGS[@]} -gt 0 ]; then
            printf "     ${DIM}%s${NC}\n" "${DRY_PKGS[@]}"
        fi
        info_kv "[DRY-RUN] 将启用服务" "${#DRY_SVCS[@]} 项"
    ...
    echo ""
    info_kv "日志" "$TEMP_LOG_FILE"
    info_kv "进度文件" "$STATE_FILE" "(全部完成后可删除)"
...
do_restore() {
    ...
    section "Restore" "重现 niri 桌面环境"
    [ "$DRY_RUN" -eq 1 ] && warn "DRY-RUN 模式：只打印计划，不实际改动系统。"
...
do_rollback() {
    ...
    section "恢复中" "$snapshot"
    if ! confirm "确认从快照 ${snapshot} 恢复配置？[y/N] (默认 N, 15s):" "N" 15; then
        log "取消回滚。"
        return
    fi
    ...
    exe tar xzf "$tgz" -C "$workdir" || { error "解包快照失败。"; exit 1; }
