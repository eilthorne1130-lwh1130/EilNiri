...
- **hyprlock / hypridle 源码编译兜底**：Ubuntu 26.04+ 的 universe 已自带 hyprlock/hypridle（直接 apt 装）；**Debian 13 / 旧版 Ubuntu / AlmaLinux 10 无包时，restore 自动从源码构建**。当前 hyprlock/hypridle 使用 CMake/Ninja。Debian 系会**硬校验 C++ 工具链**（`build-essential`/`g++`/`cmake`/`ninja`，要求 C++23）；无 C++26 时（Debian 12 / Ubuntu 22.04 的 gcc 12）自动钉到已知 C++23 的 hypr* 发行 tag。C++ 栈按依赖顺序源码构建：`hyprwayland-scanner` → `hyprutils` → `hyprlang` → **`hyprland-protocols`**（hypridle 硬依赖，旧脚本漏编）；`hyprgraphics` 仅 hyprlock 需要。栈失败即中止，不再 warn 后硬编。源码构建的 hyprlock 会**自动写入 `/etc/pam.d/hyprlock`**。此兜底由 `SOURCE_PKGS` 表驱动。
