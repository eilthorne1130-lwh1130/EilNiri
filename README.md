...
- **hyprlock / hypridle 源码编译兜底**：Ubuntu 26.04+ 的 universe 已自带 hyprlock/hypridle（直接 apt 装）；**Debian 13 / 旧版 Ubuntu / AlmaLinux 10 无包时，restore 自动从源码构建**。当前 hyprlock/hypridle 使用 CMake/Ninja，不再错误调用 cargo；RHEL 构建前安装 `pam-devel`、`sdbus-cpp-devel`，并构建 `hyprwayland-scanner`、`hyprutils`、`hyprlang`、`hyprgraphics` 依赖链。源码构建的 hyprlock 会**自动写入 `/etc/pam.d/hyprlock`**。此兜底由 `SOURCE_PKGS` 表驱动。
